    ...
    <section id="realisations" className="bg-page">
      <div className="mx-auto max-w-container px-6 py-24">
        <div className="mx-auto mb-14 flex max-w-[620px] flex-col gap-3.5 text-center">
          <div className="text-[12.5px] font-bold uppercase tracking-[1.5px] text-brand">Galerie</div>
          ...
          {gallery.map((g) => (
            <RevealOnScroll key={g.src}>
              <button
                type="button"
                onClick={() => setActive(g)}
                className="group relative block aspect-[4/3] w-full cursor-zoom-in overflow-hidden rounded-xl border-[0.5px] border-hairline bg-page transition-all duration-[180ms] hover:-translate-y-[3px] hover:shadow-[0_10px_24px_rgba(0,0,0,.14)]"
              >
                {g.isVideo ? (
                  <>
                    <video src={g.src} muted preload="metadata" className="h-full w-full object-cover" />
